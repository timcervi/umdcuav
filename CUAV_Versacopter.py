...
class AsyncServoControl (threading.Thread):
	...
	def run(self):
		...
		maxstep = 50
		sonar_reading = 765 # just initialize this to max
		...
		while RunThreadsFlag:
			...
			sonarcountdown -= 1
			if (sonarcountdown == 0) and UseSonar:
				try:
				...
				sonar_reading = cval
				if NetEnable and airborne and ((cval >= 20) and (cval < sonar_reading_threshold) and trackp and not netfired):  # need to add 'and airborne'
